...
# Noted in class that we need https:// prefix
wikipedia_web_page <- 
    getURLContent('https://en.wikipedia.org/wiki/Spirit_Airlines')
...
# xts is a feature of package xts, which extends R's native time-series 
# capabilities. From Authors: "Easily convert one of R's many time-series
# (and non-time-series) classes to a true time-based object
# which inherits all of zoo's methods, while allowing for new time-based
# tools where appropriate. Additionally, one may use xts to create new
# objects which can contain arbitrary attributes named during creation 
# as name=value pairs.
...
# Before plotting find the min and max value for both Ys 
icsmin = min(modeling_data_frame$ICS)
nhsmin = min(modeling_data_frame$NHS) 
icsmax = max(modeling_data_frame$ICS)
nhsmax = max(modeling_data_frame$NHS)
plotting_object <- ggplot(modeling_data_frame, aes(x = Year, y = ICS), color="green") + 
     geom_line() +
    geom_line(aes(y=NHS), color="red") +
    annotate("rect", xmin = 2007.917, xmax = 2009.417, 
        ymin = min(c(icsmin, nhsmin)), 
        ymax = max(c(icsmax, nhsmax)), 
        alpha = 0.3, fill = "red") +
...
# During class there was a problem finding the FRED code for Real GDP.
# on the FRED website, search for any indicator series of interest. When you
# find what you want, click on it to produce a graph. You will find the 
# code for the particular series at the top of the graph. For example, Real
# GDP Per capita has the non-obvious code "A939RX0Q048SBEA". Use it as shown here:

getSymbols("A939RX0Q048SBEA",src="FRED",return.class = "xts")
print(str(A939RX0Q048SBEA)) # show the structure of this xtx time series object
# plot the series
chartSeries(A939RX0Q048SBEA,theme="white")
